...
| **Smart selector engine with intent-aware ranked resolution** | Explicit selectors stay strict; human text resolves through actionable/accessibility heuristics with ambiguity detection instead of blindly clicking the first match. Means stories fail safely and stay maintainable when UI shifts. | MEDIUM | Borrow Playwright-style locator heuristics, but keep explicit DSL intent as a hard contract. |
