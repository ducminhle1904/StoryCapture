 ...
 */
async launchAutomation(storySource: string, projectFolder: string, onEvent: TAURI_CHANNEL<AutomationEvent>, chromeHiding: boolean | null, pacingProfile: PacingProfileDto | null, recordingSessionId: string | null, recordingDisplay: RecordingDisplayPlacementDto | null) : Promise<Result<null, AppError>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("launch_automation", { storySource, projectFolder, onEvent, chromeHiding, pacingProfile, recordingSessionId, recordingDisplay }) };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
