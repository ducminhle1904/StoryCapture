// @ts-nocheck
// Checked-in host IPC compatibility surface.
//
...
function __makeEvents__<T extends Record<string, unknown>>(mappings: Record<keyof T, string>) {
  return new Proxy(
    {} as unknown as {
    ...
    },
    {
      get: (_, event) => {
        ...
        const eventTarget = (() => {}) as (handle: __WebviewWindow__) => __EventObj__<T[keyof T]>;

        return new Proxy(eventTarget, {
          apply: (_, __, [window]: [__WebviewWindow__]) => ({
            listen: (arg: TAURI_API_EVENT.EventCallback<T[keyof T]>) => window.listen(name, arg),
            once: (arg: TAURI_API_EVENT.EventCallback<T[keyof T]>) => window.once(name, arg),
            emit: (arg: T[keyof T]) => window.emit(name, arg),
          }),
          get: (_, command: keyof __EventObj__<T[keyof T]>) => {
            switch (command) {
              case "listen":
                return (arg: TAURI_API_EVENT.EventCallback<T[keyof T]>) =>
                  TAURI_API_EVENT.listen(name, arg);
              case "once":
                return (arg: TAURI_API_EVENT.EventCallback<T[keyof T]>) =>
                  TAURI_API_EVENT.once(name, arg);
              case "emit":
                return (arg: T[keyof T]) => TAURI_API_EVENT.emit(name, arg);
            }
          },
        });
      },
