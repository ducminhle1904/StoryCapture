...
export const storeSecret = (
  ...
  key: string,
  value: string,
): Promise<void> => invoke<void>("store_secret", { service, key, value });

export const loadSecret = (
  service: string,
  key: string,
): Promise<string> => invoke<string>("load_secret", { service, key });
 ...
 */
export const onPanic = (
  cb: (payload: PanicPayload) => void,
): Promise<UnlistenFn> =>
