...
//
// State ownership lives in tokio mpsc actors, NOT in
...
// `mpsc::Sender` handles that downstream actors populate when they spawn.
...
/// (e.g. `"automation"`, `"capture"`, `"encoder"`); values are erased
/// mpsc senders. Erasure (`serde_json::Value`) is intentional: each
/// consumer downcasts via its own typed wrapper. Senders SHOULD also
/// expose a typed accessor on `AppState` (e.g.
...
pub struct AppState {
    /// Resolved at startup from `app.path().app_data_dir()`. Storage
    /// puts the global SQLite + per-project databases here.
    pub data_dir: PathBuf,

    /// Resolved at startup from `app.path().app_log_dir()`.
    /// `tracing-subscriber` rolls files in this dir.
    pub log_dir: PathBuf,
    ...
    /// Playwright driver slot dedicated to the live-preview pump.
    /// Populated when a recording against a Playwright auto-target
    ...
    /// Join handle of the task draining the watch channel and emitting
    /// `preview://frame` events. `Some` while a stream is active;
    /// aborted and replaced on start/stop.
    pub preview_pump: TokioMutex<Option<tokio::task::JoinHandle<()>>>,

    /// Author-time preview sessions keyed by streamId. Each entry owns a
    /// dedicated Playwright sidecar process (separate from the recording
    /// session) plus the pump task emitting that stream's frames.
...
/// hands out an `Arc<PlaywrightSidecarDriver>` so the simulator can run
/// DSL verbs against the same session without spawning a third Chromium.
///
...
impl AppState {
    ...
    /// Called by the host during project-open.
