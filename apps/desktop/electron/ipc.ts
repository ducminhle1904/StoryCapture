...
import electronUpdater, { type UpdateCheckResult, type UpdateInfo as ElectronUpdateInfo } from "electron-updater";
import slugify from "@sindresorhus/slugify";
import ffmpegPath from "ffmpeg-static";
import { spawn, type ChildProcessWithoutNullStreams } from "node:child_process";
import { constants as fsConstants, watch as watchFs, type FSWatcher, type Stats } from "node:fs";
...
type InvokeArgs = Record<string, unknown> | Uint8Array | ArrayBuffer | null | undefined;
...
type CaptureTarget =
  ...
  | { kind: "display_region"; display_id: number | string; rect: { x: number; y: number; w: number; h: number } };
...
type PickResult =
  | {
      ...
      candidates: Array<{ kind: string; value: unknown; score: number; unique: boolean; nth?: number }>;
...
function webBaseUrl(): string {
  return (process.env.STORYCAPTURE_WEB_URL ?? (app.isPackaged ? "https://storycapture.app" : "http://localhost:3000")).replace(/\/+$/, "");
}
...
async function logFromFrontend(payload: FrontendLogPayload): Promise<null> {
  ...
  const fields = Array.isArray(payload.fields)
    ? payload.fields
        .map(([key, value]) => `${sanitizeLogField(key)}=${JSON.stringify(sanitizeLogField(value))}`)
        .join(" ")
    : "";
...
function shellOptions(value: unknown): { cwd?: string; env?: NodeJS.ProcessEnv; encoding: BufferEncoding } {
...
async function restoreElectronWindowState(): Promise<void> {
  const state = await readJson<{ windows?: Array<{ bounds?: Rectangle; maximized?: boolean; fullscreen?: boolean }> }>(
    windowStatePath(),
    {},
  );
...
async function storeGenericSecret(service: unknown, account: unknown, value: unknown): Promise<null> {
  assertSafeStorage();
  ...
  store.keys[genericSecretKey(service, account)] = safeStorage.encryptString(secretValue).toString("base64");
...
async function loadOptionalGenericSecret(service: unknown, account: unknown): Promise<string | null> {
  try {
...
async function startWebOauth(): Promise<number> {
  ...
  const server = createServer((request, response) => {
    ...
    if (token) {
      ...
      response.end("<html><body><h1>Authentication successful</h1><p>You can close this window and return to StoryCapture.</p></body></html>");
      resolveToken(token);
    } else {
      response.writeHead(400, { "content-type": "text/html", connection: "close" });
      response.end("<html><body><h1>Authentication failed</h1><p>No token received. Please try again.</p></body></html>");
  ...
  try {
    await shell.openExternal(`${webBaseUrl()}/api/auth/signin/github?callbackUrl=http://localhost:${port}/callback`);
  } catch (error) {
    closePendingOAuthFlow();
    throw error;
  }
...
async function queueWebSyncItem(desktopId: string, workspaceId: string, payload: unknown): Promise<void> {
  const queue = await readWebSyncQueue();
  queue.push({ id: randomUUID(), desktopId, workspaceId, payload, createdAt: new Date().toISOString() });
  await writeWebSyncQueue(queue);
}

async function postTrpcMutation(token: string, procedure: string, payload: unknown): Promise<Record<string, unknown>> {
...
function updateUploadStatus(progress: UploadProgressEvent | null, sender?: WebContents, channelId?: number | null): void {
...
async function uploadVideo(args: Record<string, unknown>, sender: WebContents) {
  ...
  updateUploadStatus({ phase: "thumbnail", partNumber: 0, totalParts: 0, bytesUploaded: 0, totalBytes }, sender, onProgress);
  ...
  if (typeof args.sceneBoundaries === "string") body.sceneBoundaries = JSON.parse(args.sceneBoundaries);
  ...
  if (!initiate.ok) throw new Error(`initiate failed: ${initiate.status} - ${await initiate.text()}`);
  ...
  if (!videoId || !uploadId || !r2Key || !slug) throw new Error("initiate failed: invalid server response");

  const totalParts = totalBytes < UPLOAD_MIN_MULTIPART_SIZE ? 1 : Math.ceil(totalBytes / UPLOAD_CHUNK_SIZE);
  ...
  try {
    for (let partNumber = 1; partNumber <= totalParts; partNumber += 1) {
      ...
      if (!presign.ok) throw new Error(`presign failed for part ${partNumber}: ${await presign.text()}`);
      ...
      updateUploadStatus({ phase: "uploading", partNumber, totalParts, bytesUploaded, totalBytes }, sender, onProgress);
    }
  } finally {
    await file.close();
  }
  ...
  if (uploadCancelRequested) throw new Error("upload cancelled");
  updateUploadStatus({ phase: "completing", partNumber: totalParts, totalParts, bytesUploaded: totalBytes, totalBytes }, sender, onProgress);
...
function providerId(raw: unknown): ProviderId {
  ...
  if (value === "anthropic" || value === "openai" || value === "elevenlabs" || value === "openai_tts") {
    return value;
  }
...
function providerProbe(provider: ProviderId) {
  switch (provider) {
    case "anthropic":
      return { url: "https://api.anthropic.com/v1/models", header: "x-api-key", value: (key: string) => key };
    case "elevenlabs":
      return { url: "https://api.elevenlabs.io/v1/voices", header: "xi-api-key", value: (key: string) => key };
    case "openai":
    case "openai_tts":
      return { url: "https://api.openai.com/v1/models", header: "Authorization", value: (key: string) => `Bearer ${key}` };
  }
}
...
async function keyTest(provider: ProviderId) {
  ...
  try {
    ...
    throw new Error(`network error contacting provider: ${error instanceof Error ? error.message : String(error)}`);
  }
}
...
async function createProject(raw: unknown): Promise<ProjectRecord> {
  ...
  if (args.workflow_type && args.workflow_state && args.workflow_type !== args.workflow_state.type) {
  ...
  await fs.writeFile(paths.storyPath, args.starter_story_source ?? defaultStarterStory(name), "utf8");
...
async function updateProjectWorkflow(id: string, workflowState: WorkflowState): Promise<WorkflowState> {
  const project = await findProject(id);
...
async function readRecordingSidecar(recordingPath: string, suffix: "actions" | "trajectory" | "steps") {
...
async function exportRun(args: ExportRunArgs) {
  ...
  const snapshotPath = path.join(args.output_folder, `${slugify(args.base_name || args.story_id || "export") || "export"}.graph.json`);
  ...
  for (const [index, output] of args.outputs.entries()) {
    ...
    if (output.format === "mp4") {
      ffmpegArgs.push("-c:v", "libx264", "-preset", "veryfast", "-pix_fmt", "yuv420p", "-movflags", "+faststart", "-an");
    } else if (output.format === "webm") {
      ffmpegArgs.push("-c:v", "libvpx-vp9", "-b:v", output.quality === "high" ? "4M" : output.quality === "med" ? "2M" : "1M", "-an");
    }
    ffmpegArgs.push(out);
...
function renderListActive(storyId: string): RenderJob[] {
  return [...renderSessions.values()]
    ...
    .filter((job) => job.story_id === storyId && job.status !== "completed" && job.status !== "cancelled")
...
function cropDisplayRegionThumbnail(image: NativeImage, target: Extract<CaptureTarget, { kind: "display_region" }>): NativeImage {
...
async function captureTargetThumbnail(target: CaptureTarget, maxWidth: number, maxHeight: number) {
  ...
  const sourceTypes = target.kind === "window" || target.kind === "window_by_pid" ? ["window"] : ["screen"];
  const sources = await desktopCapturer.getSources({
  ...
  const image = target.kind === "display_region"
...
function snapshotEntry(url: string, html: string, htmlPath: string, screenshotPath: string): AuthorSnapshotEntry {
...
async function authorSnapshotGet(projectDir: string, url: string): Promise<AuthorSnapshotEntry | null> {
  ...
  try {
    ...
    const snapshots = await Promise.all(
      entries
        ...
        .map(async (entry) => {
          try {
            return JSON.parse(await fs.readFile(path.join(dir, entry.name), "utf8")) as AuthorSnapshotEntry;
          } catch {
            return null;
          }
        }),
...
async function authorSnapshotCapture(projectDir: string, url: string): Promise<AuthorSnapshotEntry> {
  const dir = snapshotDir(projectDir);
  ...
  try {
    ...
    const html = await win.webContents.executeJavaScript("document.documentElement.outerHTML", true) as string;
...
function displayForTarget(target: CaptureTarget) {
  ...
  if (target.kind === "display" || target.kind === "display_region") {
    return displays.find((display) => Number(display.id) === Number(target.display_id)) ?? displays[0];
  }
  return displays[0];
}
...
async function pumpCaptureStreamFrame(session: CaptureStreamSession): Promise<void> {
  ...
  try {
    ...
    sendChannel(session.sender, session.eventChannelId, captureEventJson("frame-delivered", {
      sequence: frameSequence,
    ...
    sendChannel(session.sender, session.eventChannelId, captureEventJson("frame-dropped", {
      sequence: frameSequence,
...
function dialogButtonPlan(spec: DialogButtonSpec | null | undefined): {
  ...
  if (spec === "OkCancel") {
    return { buttons: ["OK", "Cancel"], cancelId: 1, result: (response) => response === 0 ? "Ok" : "Cancel" };
  }
  if (spec === "YesNo") {
    return { buttons: ["Yes", "No"], cancelId: 1, result: (response) => response === 0 ? "Yes" : "No" };
  }
  if (spec === "YesNoCancel") {
    return { buttons: ["Yes", "No", "Cancel"], cancelId: 2, result: (response) => response === 0 ? "Yes" : response === 1 ? "No" : "Cancel" };
  }
  if (spec && typeof spec === "object" && "OkCancelCustom" in spec && Array.isArray(spec.OkCancelCustom)) {
    const labels = spec.OkCancelCustom;
    return { buttons: labels, cancelId: 1, result: (response) => response === 0 ? "Ok" : "Cancel" };
  }
  if (spec && typeof spec === "object" && "YesNoCancelCustom" in spec && Array.isArray(spec.YesNoCancelCustom)) {
    const labels = spec.YesNoCancelCustom;
    return { buttons: labels, cancelId: 2, result: (response) => response === 0 ? "Yes" : response === 1 ? "No" : "Cancel" };
  }
...
async function captureRecordingFrame(session: RecordingSession): Promise<void> {
  ...
  const framePath = path.join(session.framesDir, `frame-${String(frameIndex).padStart(6, "0")}.png`);
  try {
    const bytes = await captureTargetThumbnail(session.target, session.width, session.height);
...
function ffmpegCropPlan(crop: FrameCropRect | null, frameWidth: number, frameHeight: number): {
  filter: string;
  width: number;
  height: number;
} | null {
...
function authorSession(streamId: string): AuthorPreviewSession {
  ...
  if (!session || session.window.isDestroyed()) throw new Error(`author preview ${streamId} not found`);
  return session;
}
...
async function startAuthorPreviewSession(args: Record<string, unknown>, sender: WebContents): Promise<string> {
  ...
  const initialUrl = typeof args.initialUrl === "string" && args.initialUrl.length > 0
    ? args.initialUrl
    : "about:blank";
...
async function pickerStampStepId(raw: Record<string, unknown>) {
  ...
  const lineHasCommand = Boolean(
    ast?.scenes?.some((scene) => scene.commands?.some((command) => command.span.line === lineOffset)),
  );
...
async function updateSettingsField(field: "browser_executable" | "browser_language", value: unknown) {
  const base = await getSettings();
...
function fsInvokeOptions(
  ...
  if (
    ...
    return rawOptions && typeof rawOptions === "object" ? rawOptions as Record<string, unknown> : {};
  }
...
function fsFileResource(rid: unknown): FsFileResource {
  ...
  if (resource.kind !== "file") throw new Error(`Filesystem rid is not a file handle: ${String(rid)}`);
  return resource;
}

function fsOpenFlags(options: Record<string, unknown>): { append: boolean; flags: number; mode?: number } {
  ...
  let flags = read && write ? fsConstants.O_RDWR : write ? fsConstants.O_WRONLY : fsConstants.O_RDONLY;
...
function parseTarget(raw: string): unknown {
  const text = raw.trim();
  const prefixed = text.match(/^(selector|testid|aria|label|text|text_exact)\s+(.+)$/);
  if (prefixed) {
    const kindMap: Record<string, string> = { testid: "test_id" };
    return { kind: kindMap[prefixed[1]] ?? prefixed[1], value: stripQuotes(prefixed[2]) };
  }
  const role = text.match(/^<([a-zA-Z][a-zA-Z0-9_-]*)>\s+(.+)$/);
  if (role) {
    return { kind: "role", value: { role: role[1], name: stripQuotes(role[2]) } };
  }
  return { kind: "text", value: stripQuotes(text) };
}
...
function publishLspDiagnostics(sender: WebContents, channelId: number | null, uri: string, text: string): void {
...
function lspDidChange(params: unknown, sender: WebContents, channelId: number | null): void {
  ...
  if (!uri) return;
  const changes = (params as { contentChanges?: Array<{ text?: unknown }> } | null)?.contentChanges ?? [];
  const text = typeof changes.at(-1)?.text === "string" ? String(changes.at(-1)?.text) : (lspDocuments.get(uri)?.text ?? "");
  const version = Number((params as { textDocument?: { version?: unknown } } | null)?.textDocument?.version ?? lspDocuments.get(uri)?.version ?? 1);
...
function handleLspRequest(args: Record<string, unknown>, sender: WebContents): string {
  ...
  try {
    ...
    return lspError(null, -32700, `invalid JSON-RPC envelope: ${error instanceof Error ? error.message : String(error)}`);
  }
  ...
  try {
    switch (method) {
      ...
      default:
        return id == null ? "null" : lspError(id, -32601, `method not found: ${method || "unknown"}`);
    }
  } catch (error) {
    return id == null
      ? "null"
...
function parseStorySource(source: string) {
  ...
  const story = {
    ...
    scenes: [] as Array<{ name: string; commands: unknown[]; span: { start: number; end: number; line: number; col: number } }>,
  ...
  lines.forEach((line, index) => {
    ...
    if (!currentScene) {
      diagnostics.push({ severity: "warning", message: "command outside scene ignored", span, suggestion: null });
      return;
    }
    ...
    if (verb === "click" || verb === "hover" || verb === "assert") {
      command = { verb, target: parseTarget(rest), ...base };
    }
    if (verb === "type" || verb === "select" || verb === "upload") {
      const match = rest.match(/^(.+?)\s+(?:with\s+)?("[^"]*"|'[^']*'|.+)$/);
      command = {
        verb,
        target: parseTarget(match?.[1] ?? rest),
        [verb === "upload" ? "path" : verb === "select" ? "value" : "text"]: stripQuotes(match?.[2] ?? ""),
        ...base,
      };
    }
    if (verb === "scroll") {
      command = { verb, direction: parts[1] ?? "down", amount: parts[2] ? Number(parts[2]) : null, ...base };
    }
    if (verb === "wait") command = { verb, duration_ms: parseDurationMs(rest), ...base };
    if (verb === "wait-for") command = { verb, target: parseTarget(rest), timeout_ms: null, ...base };
    if (verb === "screenshot") command = { verb, name: stripQuotes(rest || `shot-${lineNo}`), ...base };
    if (verb === "pause") command = { verb, ...base };
    if (command) {
      currentScene.commands.push(command);
    } else if (!["{", "}"].includes(trimmed)) {
      diagnostics.push({ severity: "error", message: `unknown command: ${verb}`, span, suggestion: null });
    }
  });
...
async function elementCenter(contents: WebContents, target: unknown): Promise<{ x: number; y: number } | null> {
  ...
  return contents.executeJavaScript(`
    (() => {
      ...
      if (!el && target && typeof target.value === 'string') {
        const needle = target.value.toLowerCase();
        el = [...document.querySelectorAll('button,a,input,textarea,select,[role],label,[aria-label]')]
          .find((candidate) => {
...
async function executeParsedCommand(
  ...
  if ((command.verb === "click" || command.verb === "hover") && center) {
    ...
    if (command.verb === "click") {
      contents.sendInputEvent({ type: "mouseDown", x: center.x, y: center.y, button: "left", clickCount: 1 });
      contents.sendInputEvent({ type: "mouseUp", x: center.x, y: center.y, button: "left", clickCount: 1 });
    }
  ...
  if ((command.verb === "type" || command.verb === "select") && center) {
    contents.sendInputEvent({ type: "mouseDown", x: center.x, y: center.y, button: "left", clickCount: 1 });
    contents.sendInputEvent({ type: "mouseUp", x: center.x, y: center.y, button: "left", clickCount: 1 });
    if (command.verb === "type") contents.sendInputEvent({ type: "char", keyCode: command.text ?? "" });
  ...
  if (command.verb === "screenshot") {
    ...
    await fs.mkdir(exportsDir, { recursive: true });
    const safeName = slugify(command.name ?? `screenshot-${Date.now()}`) || `screenshot-${Date.now()}`;
...
async function launchAutomationCommand(args: Record<string, unknown>, sender: WebContents) {
  ...
  sendChannel(sender, onEvent, { json: JSON.stringify({ type: "story_started", story_hash: storyHash(source) }) });
  sendChannel(sender, onEvent, { json: JSON.stringify({ type: "scene_entered", name: "Electron preview", ordinal: 1 }) });
  ...
  for (const [index, command] of commands.entries()) {
    ...
    sendChannel(sender, onEvent, { json: JSON.stringify({ type: "step_started", ordinal, command, driver_used: "electron" }) });
    const started = Date.now();
    try {
  ...
  sendChannel(sender, onEvent, {
    json: JSON.stringify({
      type: "story_ended",
      status: { total_steps: commands.length, succeeded, failed, duration_ms: commands.length * 50 },
    }),
  });
  const recordingSessionId = typeof args.recordingSessionId === "string" ? args.recordingSessionId : null;
...
async function readTargetsForStory(storyPath: string): Promise<{ version: number; steps: Record<string, { primary?: unknown; fallbacks?: unknown[] }> }> {
  const targets = await readJson<{ version: number; steps: Record<string, { primary?: unknown; fallbacks?: unknown[] }> }>(
    targetsPathFor(storyPath),
    { version: 1, steps: {} },
  );
  return { version: 1, steps: targets.steps ?? {} };
}

async function writeTargetsForStory(storyPath: string, targets: { version: number; steps: Record<string, unknown> }): Promise<void> {
  const targetsPath = targetsPathFor(storyPath);
  const tempPath = `${targetsPath}.tmp.${process.pid}`;
...
async function simulatorStartCommand(args: Record<string, unknown>, sender: WebContents): Promise<string> {
  const id = randomUUID();
  ...
  simulatorSessions.set(id, { id, sender, channelId, storyPath, commands, frames, totalSteps, cancelled: false });
  sendChannel(sender, channelId, { type: "started", session_id: id, run_id: runId, total_steps: totalSteps });
...
async function simulatorPromoteFallback(sessionId: string, ordinal: number): Promise<null> {
  ...
  const [promoted, ...remainingFallbacks] = Array.isArray(stepTargets?.fallbacks) ? stepTargets.fallbacks : [];
  ...
  targets.steps[command.step_id] = {
    ...
    fallbacks: [normalizedTargetRecord(stepTargets.primary), ...remainingFallbacks.map(normalizedTargetRecord)],
  };
  await writeTargetsForStory(session.storyPath, targets);
  session.frames.set(ordinal, { ...frame, match_kind: "primary", matched_selector: selectorSummary(promoted) });
  return null;
}
...
function dryRunStep(raw: unknown, index: number): DryRunStep {
  ...
  const stepArgs = record.args && typeof record.args === "object" ? (record.args as Record<string, unknown>) : {};
  return {
...
async function handleInvoke(
  ...
  switch (cmd) {
    ...
    case "stop_recording":
      return stopRecording((args as { session?: { id?: string } | undefined } | undefined)?.session);
    ...
    case "stop_author_preview":
      return stopAuthorPreviewSession(String((args as { streamId?: string } | undefined)?.streamId ?? ""));
    case "pause_author_preview":
      authorSession(String((args as { streamId?: string } | undefined)?.streamId ?? "")).paused = true;
      return null;
    case "resume_author_preview":
      authorSession(String((args as { streamId?: string } | undefined)?.streamId ?? "")).paused = false;
      return null;
    case "set_author_preview_viewport": {
      const payload = args as { streamId?: string; args?: { width?: number; height?: number } } | undefined;
    ...
    case "author_preview_back":
      authorSession(String((args as { streamId?: string } | undefined)?.streamId ?? "")).window.webContents.navigationHistory.goBack();
      return null;
    case "author_preview_forward":
      authorSession(String((args as { streamId?: string } | undefined)?.streamId ?? "")).window.webContents.navigationHistory.goForward();
      return null;
    case "author_preview_reload":
      authorSession(String((args as { streamId?: string } | undefined)?.streamId ?? "")).window.webContents.reload();
      return null;
    ...
    case "author_dispatch_input":
      dispatchAuthorInput(
        ...
        ((args as { event?: Record<string, unknown> } | undefined)?.event ?? {}) as Record<string, unknown>,
      );
      return null;
    ...
    case "picker_start": {
      ...
      return pickerStartAuthor({ ...(args as Record<string, unknown> | undefined), streamId: first });
    }
    case "picker_cancel":
      return pickerCancel();
    ...
    case "author_snapshot_list":
      return authorSnapshotList(String((args as { projectDir?: unknown; project_dir?: unknown } | undefined)?.projectDir ?? (args as { projectDir?: unknown; project_dir?: unknown } | undefined)?.project_dir ?? ""));
    case "author_snapshot_get":
      return authorSnapshotGet(
        String((args as { projectDir?: unknown; project_dir?: unknown } | undefined)?.projectDir ?? (args as { projectDir?: unknown; project_dir?: unknown } | undefined)?.project_dir ?? ""),
    ...
    case "tts_apply_sync":
      return ttsApplySyncWithoutCachedClips((args as { stepTimings?: unknown } | undefined)?.stepTimings);
    case "tts_gc_cache":
      return 0;
    ...
    case "store_secret": {
      const payload = args as { service?: unknown; account?: unknown; key?: unknown; value?: unknown } | undefined;
    ...
    case "remove_project":
      return removeProject(String((args as { args?: { id?: string } } | undefined)?.args?.id ?? ""));
    case "get_project_workflow":
      return getProjectWorkflow(String((args as { args?: { id?: string } } | undefined)?.args?.id ?? ""));
    case "update_project_workflow": {
      const payload = (args as { args?: { id?: string; workflow_state?: WorkflowState } } | undefined)?.args;
    ...
    case "get_recording_actions":
      return readRecordingSidecar(
        String((args as { args?: { recording_path?: string } } | undefined)?.args?.recording_path ?? ""),
        "actions",
      );
    ...
    case "export_validate_config":
      validateExportOutput((args as { cfg?: ExportOutput } | undefined)?.cfg ?? ({} as ExportOutput));
      return null;
    case "export_run":
      return exportRun((args as { args?: ExportRunArgs } | undefined)?.args ?? ({} as ExportRunArgs));
    case "list_displays":
      return displayInfo();
    case "list_windows":
      return windowInfo();
    ...
    case "set_capture_target":
      await writeJson(captureTargetPath(), (args as { target?: CaptureTarget } | undefined)?.target ?? null);
      return null;
    case "capture_target_thumbnail": {
      ...
      return captureTargetThumbnail(payload.target, payload.maxWidth ?? 320, payload.maxHeight ?? 180);
    }
    ...
    case "plugin:dialog|open": {
      ...
      const result = await dialog.showOpenDialog({
        ...
        ].filter((property): property is "openDirectory" | "openFile" | "multiSelections" | "createDirectory" =>
          property === "openDirectory" || property === "openFile" || property === "multiSelections" || property === "createDirectory",
        ),
      });
    ...
    case "plugin:dialog|save": {
      ...
      const result = await dialog.showSaveDialog({
        ...
        ].filter((property): property is "createDirectory" => property === "createDirectory"),
      });
    ...
    case "plugin:log|log": {
      const payload = args as
        | { level?: unknown; message?: unknown; location?: unknown; file?: unknown; line?: unknown; keyValues?: unknown }
        | undefined;
      return logFromFrontend({
    ...
    case "plugin:updater|check": {
      ...
      return pendingUpdateCheck?.updateInfo && update ? updaterMetadata(pendingUpdateCheck.updateInfo) : null;
    }
    case "plugin:updater|download": {
      const rid = (args as { rid?: unknown } | undefined)?.rid;
      if (typeof rid !== "number" || !updaterResources.has(rid)) throw new Error("unknown update resource");
    ...
    case "plugin:shell|spawn": {
      const payload = args as { program?: unknown; args?: unknown; options?: unknown; onEvent?: unknown } | undefined;
      const program = String(payload?.program ?? "");
      ...
      child.stdout.on("data", (chunk: Buffer) => {
        sendChannel(event.sender, channelId, { event: "Stdout", payload: chunk.toString(options.encoding) });
      });
      child.stderr.on("data", (chunk: Buffer) => {
        sendChannel(event.sender, channelId, { event: "Stderr", payload: chunk.toString(options.encoding) });
      });
    ...
    case "plugin:fs|seek": {
      ...
      const base = whence === 1 ? resource.position : whence === 2 ? (await resource.handle.stat()).size : 0;
    ...
    case "plugin:fs|write": {
      ...
      resource.position = resource.append ? (await resource.handle.stat()).size : resource.position + bytesWritten;
      return bytesWritten;
    }
    case "plugin:fs|exists":
      return pathExists(pathFromFsArgs(args, options));
    case "plugin:fs|watch": {
      const payload = args as { paths?: unknown; onEvent?: unknown; options?: { recursive?: boolean } } | undefined;
      ...
      const watchers = watchPaths.map((watchPath) => {
        ...
        try {
          return watchFs(watchPath, { recursive: payload?.options?.recursive === true }, (_type, changedPath) => {
...
export function registerIpcHandlers(): void {
  ipcMain.handle("tauri-invoke", async (event, envelope: InvokeEnvelope) => {
    return handleInvoke(event, envelope);
  });
}
