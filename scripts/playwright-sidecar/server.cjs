...
var handlers = {
  ...
  launch: async (params) => {
    ...
    state.browser = await import_playwright_core.chromium.launch({ headless: headless !== false });
  ...
  scroll: async ({ direction, amount }) => {
    const px = amount || 400;
    const [x, y] = direction === "down" ? [0, px] : direction === "up" ? [0, -px] : direction === "right" ? [px, 0] : [-px, 0];
  ...
  waitFor: async ({ target, timeoutMs }) => {
    if (target.kind === "text" && target.value && target.value.startsWith("download:")) {
      ...
      const dest = state.downloadDir != null ? `${state.downloadDir}/${download.suggestedFilename()}` : await download.path();
  ...
  elementState: async ({ selector }) => {
    const result = await state.page.evaluate((sel) => {
      ...
      const visible = s.visibility !== "hidden" && s.display !== "none" && parseFloat(s.opacity || "1") > 0;
...
rl.on("line", async (line) => {
  ...
  try {
    ...
    write({ jsonrpc: "2.0", id, error: { code: -32e3, message: String(e && e.message || e) } });
  }
});
rl.on("close", async () => {
  if (state.browser) {
